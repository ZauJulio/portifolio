...
canonical: ""
cover: "https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/cover.webp"
repo: "https://github.com/ZauJulio/ZSOM"
---

# ZSOM: Um Mapa Auto-Organizável pronto para produção em NumPy puro

Conheci Mapas Auto-Organizáveis durante um projeto de iniciação científica na faculdade, e eles ficaram comigo o suficiente para virarem base da minha monografia. SOMs fazem algo que nenhum outro algoritmo faz tão bem: aprendem um **mapa que preserva a topologia** do espaço de alta dimensão para uma grade 2D — sem backpropagation, sem rótulos e sem função de perda para ajustar. Vizinhos na grade acabam representando entradas similares, e só essa propriedade já os torna únicos para visualização e exploração.

<br />

Pra quem quiser acompanhar pelo código, o repositório é o [**ZauJulio/ZSOM**](https://github.com/ZauJulio/ZSOM) — tudo que discuto aqui tem correspondência direta no source.

ZSOM é minha implementação do algoritmo de Kohonen do zero. NumPy puro no núcleo, Numba JIT opcional no caminho crítico e uma camada de visualização que produz figuras 4-painéis em nível de publicação e animações de treinamento. Este artigo cobre o algoritmo passo a passo, a matemática por trás de cada decisão de design, os datasets de malhas 3D e as partes que vão além do que bibliotecas SOM típicas oferecem.
...
Essa última propriedade — preservação de topologia — é o que separa SOMs de qualquer outro algoritmo de clusterização. O k-means produz k centróides desconectados sem relação espacial entre si. Um SOM produz um _mapa_ onde proximidade na grade reflete proximidade no espaço dos dados.

```mermaid
flowchart LR
  subgraph IN["Espaço de entrada · ℝᵈ (alta dim.)"]
    direction TB
    d1(("x"))
    d2(("x"))
    d3(("x"))
  ...
  subgraph GRID["Grade SOM · w×h protótipos (2D)"]
    direction TB
    g11["W"] --- g12["W"] --- g13["W"]
    g21["W"] --- g22["W"] --- g23["W"]
    g11 --- g21
  ...
  IN ==>|"aprende um mapa que preserva a topologia"| GRID
  GRID -.-> note["Vizinhos na grade ⇒ entradas semelhantes<br/>a proximidade é preservada"]

  classDef data fill:#0f766e,stroke:#2dd4bf,color:#ccfbf1;
  classDef cell fill:#1e293b,stroke:#6366f1,stroke-width:1px,color:#e2e8f0;
  classDef note fill:#312e81,stroke:#818cf8,color:#e0e7ff;
  class d1,d2,d3 data;
  class g11,g12,g13,g21,g22,g23 cell;
  class note note;
```

Cada ponto de entrada em `ℝᵈ` é mapeado para o protótipo mais próximo na grade plana e, como os nós vizinhos são puxados juntos durante o treinamento, o layout 2D acaba espelhando a estrutura dos dados. Primeiro vamos percorrer o algoritmo passo a passo, depois mergulhar na matemática por trás de cada escolha de design.

## O algoritmo, passo a passo

Todo o treinamento é um laço enxuto: escolha um vencedor, puxe sua vizinhança em direção à
amostra e então resfrie. Todo o resto deste post é uma variação de um destes cinco passos.

```mermaid
flowchart TD
  Start(["Dados de treino X · n pontos em ℝᵈ"]) --> Init["1 · Inicializar pesos W<br/>aleatório ou PCA"]
  Init --> BMU

  subgraph epoch["Uma época · para cada amostra x"]
    direction TB
    BMU["2 · Seleção do BMU — winner-takes-all<br/>bmu = argmin dist(W, x)"]
    Nbr["3 · Vizinhança<br/>h(i,j,t) = exp(−d_grid² / 2σ²)"]
    Upd["4 · Atualização dos pesos<br/>W += η·h·(x − W)"]
    BMU -->|"vencedor"| Nbr -->|"puxa vizinhos"| Upd
  end

  Upd --> More{"Mais amostras<br/>nesta época?"}
  More -->|"sim · próximo x"| BMU
  More -->|"não"| Decay["5 · Cronograma de decaimento<br/>η(t) ↓   σ(t) ↓"]
  Decay --> Done{"t = T?"}
  Done -->|"não · próxima época"| BMU
  Done -->|"sim"| Map(["Mapa que preserva a topologia"])

  classDef step fill:#1e293b,stroke:#6366f1,stroke-width:1px,color:#e2e8f0;
  classDef decision fill:#312e81,stroke:#818cf8,color:#e0e7ff;
  classDef terminal fill:#4338ca,stroke:#a5b4fc,color:#ffffff;
  class Init,BMU,Nbr,Upd,Decay step;
  class More,Done decision;
  class Start,Map terminal;
```
...
<p align="center">
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/som_grid.png"
    alt="Esquema da grade SOM"
  />
</p>
...
A função de vizinhança determina _quanto_ cada nó se move em direção a `x`, com base na distância na grade até o BMU:
...
<p align="center">
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/neighborhood_gaussian.png"
    alt="Perfis do kernel de vizinhança"
  />
</p>
...
<p align="center">
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/bmu_update_vectors.png"
    alt="Vetores de atualização do BMU e vizinhos"
  />
</p>
...
<p align="center">
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/decay_schedule.png"
    alt="Decaimento linear vs exponencial"
  />
</p>
...
A métrica de distância é a escolha de design mais fundamental de um SOM.
...
<p align="center">
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/metrics/euclidean_metric.png"
    alt="Geometria da métrica Euclidiana"
  />
</p>
...
<p align="center">
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/metrics/manhattan_metric.png"
    alt="Geometria da métrica Manhattan"
  />
</p>
...
**Quando usar:** Análise de pior caso. Tolerâncias de fabricação — uma peça está fora de especificação se _qualquer_ medida exceder o limite. Distâncias em jogos de tabuleiro (o rei move uma casa em qualquer direção — isso é distância de Chebyshev).

<p align="center">
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/metrics/chebyshev_metric.png"
    alt="Geometria da métrica Chebyshev"
  />
</p>
...
**Efeito no SOM:** O SOM aprende um mapa de _direções_, não de posições. Nós que apontam para direções semelhantes no espaço de features acabam adjacentes na grade. A magnitude do vetor é completamente ignorada.
...
<p align="center">
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/metrics/cosine_metric.png"
    alt="Geometria da métrica de Cosseno"
  />
</p>
...
<p align="center">
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/lp_unit_balls.png"
    width="25%"
    alt="Bolas unitárias Lp"
  />
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/metrics/minkowski_metric.png"
    width="28.5%"
    alt="Geometria da métrica Minkowski (p=3)"
  />
</p>
...
A inicialização aleatória espalha pesos uniformemente dentro do intervalo observado de cada dimensão de feature. Se seus dados vivem em um elipsoide fino em uma pequena região desse hipercubo, o SOM gasta as primeiras 20–40 épocas apenas _encontrando_ os dados. A inicialização por PCA coloca os pesos diretamente na variedade dos dados antes da época 1.

### A matemática

```mermaid
flowchart LR
  X(["Dados X"]) --> A["Centralizar X<br/>subtrair a média"]
  A --> B["SVD<br/>da covariância"]
  B --> C["Tomar PC1, PC2<br/>2 maiores autovetores"]
  C --> D["Construir grade w×h<br/>cobrindo ±2σ ao longo de PC1/PC2"]
  D --> E(["Mapear de volta para ℝᵈ<br/>pesos iniciais W"])

  E -.->|"já sobre a<br/>variedade dos dados"| Fast["≈ a época 0 parece<br/>a época 20–40 do aleatório"]

  classDef step fill:#1e293b,stroke:#6366f1,stroke-width:1px,color:#e2e8f0;
  classDef terminal fill:#4338ca,stroke:#a5b4fc,color:#ffffff;
  classDef good fill:#166534,stroke:#4ade80,color:#dcfce7;
  class A,B,C,D step;
  class X,E terminal;
  class Fast good;
```
...
<p align="center">
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/pca_init_vs_random.png"
    alt="Inicialização aleatória vs PCA"
  />
</p>
...
A taxa de aprendizado e o raio de vizinhança decaem linearmente por padrão — o cronograma clássico do SOM, que funciona bem na prática.
...
Leia como uma ramificação por época sobre como o erro se moveu:

```mermaid
flowchart TD
  E["Calcular ΔQE = MQE(t) − MQE(t−1)"] --> Q{"comparar ΔQE com ε"}
  Q -->|"ΔQE &lt; −ε<br/>queda acentuada"| Slow["decay × 0.95<br/>aproveita o momento por mais tempo"]
  Q -->|"abs(ΔQE) &lt; ε<br/>platô"| Push["decay × 1.05<br/>força para escapar do plano"]
  Q -->|"caso contrário<br/>progresso normal"| Keep["mantém o cronograma"]
  Slow --> Next(["próxima época"])
  Push --> Next
  Keep --> Next
...
<p align="center">
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/mqe_curve.png"
    alt="Curva de convergência do MQE"
  />
</p>
...
<p align="center">
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/hex_topology.png"
    alt="Topologia hexagonal"
  />
</p>
...
Para cada nó `(i,j)`, compute a distância Euclidiana média até seus vizinhos imediatos no _espaço dos dados_ (não no espaço da grade):
...
<p align="center">
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/umatrix_example.png"
    alt="Exemplo de grade U-Matrix"
  />
</p>
...
<p align="center">
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/umatrix_heatmap.png"
    alt="U-Matrix heatmap ridge"
  />
</p>
...
<p align="center">
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/umatrix_surface.png"
    alt="U-Matrix surface"
  />
</p>
...
<p align="center">
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/activation_heatmap.png"
...
O módulo de visualização do ZSOM produz uma figura em 4 painéis que mostra o progresso do treinamento por múltiplos ângulos de uma vez:
...
Os quatro painéis leem o mesmo mapa treinado por quatro lentes complementares:

<p align="center">
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/four_panel.png"
    alt="A figura de 4 painéis: dados de entrada com projeções do BMU, a grade de pesos colorida pela U-Matrix, o mapa de ativação e a superfície da U-Matrix"
  />
</p>
...
**Segmentação de clientes** — mapeie vetores de comportamento na grade. Nós adjacentes representam clientes similares; as cristas da U-Matrix são os limites dos clusters. Diferente do k-means, você não escolhe k de antemão — a estrutura das cristas indica quantos clusters existem.
...
A maioria das implementações de SOM disponíveis é ou um exemplo de brinquedo (50 linhas, uma métrica) ou um framework pesado otimizado para compatibilidade de API em vez de transparência.

Construí o ZSOM para ocupar um espaço diferente:
...
> SOMs têm 35 anos e ainda produzem as visualizações de aprendizado não supervisionado mais imediatamente legíveis que eu conheço. A U-Matrix sozinha — uma imagem 2D onde os limites de clusters são literalmente visíveis como cristas — comunica mais para um público não-ML em cinco segundos do que qualquer plot de t-SNE que eu já mostrei. Foi o primeiro algoritmo de rede neural que aprendi na faculdade, parte de um projeto de iniciação científica que inspirou minha monografia — poder revisitá-lo anos depois fez o ZSOM significar algo além de código.

> Se você quiser experimentar, o código está aberto. Dúvidas, sugestões ou coisas legais que fez com o ZSOM — fale comigo no Twitter ou no GitHub.
