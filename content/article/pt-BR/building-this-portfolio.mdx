...
description: "Um tour recurso a recurso por este portfólio: uma aplicação Vike híbrida SSG + SSR sobre React 19 e Hono, i18n bilíngue com remoção de prefixo de locale, um banco SQLite que vai dentro do build, dados estruturados via HyperJson e Markdown/MDX via HyperDown, busca full-text ao vivo, minimapa de TOC, navegação em série de artigos, sugestões ranqueadas por tag, sitemap/SEO e o deploy em Vercel + Docker."
date: "2026-06-12"
tags:
  [
    "Vike",
...
de verdade sem nenhum peso operacional. Os motores que tornam isso ergonômico —
transformar uma pasta de MDX num banco SQLite tipado e pesquisável, e uma pasta de
JSON em imports tipados e validados — são o tema dos dois próximos posts.
...
> Este portfólio é open source. Os dois motores de conteúdo, HyperDown e HyperJson,
> estão publicados no npm como `@indago/hyper-down` e `@indago/hyper-json`. Se algo
> disto for útil para você, pegue — e me conte o que você construir.
