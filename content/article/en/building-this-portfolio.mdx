...
description: "A feature-by-feature tour of this portfolio: a hybrid SSG + SSR Vike app on React 19 and Hono, bilingual locale-stripping i18n, a SQLite database that ships inside the build, structured JSON via HyperJson and Markdown/MDX via HyperDown, live full-text search, a TOC minimap, article series navigation, tag-ranked suggestions, sitemap/SEO, and the Vercel + Docker deploy."
date: "2026-06-12"
tags:
  [
    "Vike",
...
This post is the first in a three-part series. It is the **overview** — a
feature-by-feature walk through the whole site and the decisions behind it. The
next two posts drill into the two content engines I built to make it work:
[HyperDown](/articles/what-is-hyperdown) (Markdown/MDX → SQLite) and
[HyperJson](/articles/what-is-hyperjson) (JSON Schema → typed content).
...
operational weight. The engines that make it ergonomic — turning a folder of MDX
into a typed, searchable SQLite database, and a folder of JSON into validated typed
imports — are the subject of the next two posts.
...
> This portfolio is open source. The two content engines, HyperDown and HyperJson,
> are published on npm as `@indago/hyper-down` and `@indago/hyper-json`. If any of
> this is useful to you, take it — and tell me what you build.
