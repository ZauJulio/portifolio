...
<UpdateLog>
  <Update date="2026-07-12">
    Added YouTube trailers across movies and games; article repo links with JSON-LD SEO; search
    loading state with serverless warmup; carousel lightbox navigation with keyboard support.
  </Update>
  <Update date="2026-07-05">
    Added books, games, and movies sections with reviews, ratings, and accent colors; hero grid
    layout with platform icons; ImageCarousel component with infinite scroll.
  </Update>
  <Update date="2026-06-26">
    Music and photography navigation improvements; playlist pagination.
  </Update>
  <Update date="2026-06-19">
    Zoomable Mermaid diagrams; SEO refactor with canonical URLs and structured data; upgraded Indago
    engines to 1.6.0 (FTS `detail=column` + `optimize`); keyboard shortcut (Ctrl/⌘+.) for search.
  </Update>
  <Update date="2026-06-17">
    Tutorial sidebar with scroll-spy active section; in-article search (plain text for all articles,
    `#` prefix for current-page sections); reading state tracking in localStorage.
  </Update>
  <Update date="2026-06-16">
    Standardized article cards with mobile carousels; draft system for HyperDown; proper 404 pages
    with `noindex`; migrated all URLs to the production domain.
  </Update>
  <Update date="2026-06-13">
    Initial release — Vike hybrid SSG+SSR, Indago engines from npm, bilingual content, series
    navigation with tag-ranked suggestions.
  </Update>
</UpdateLog>
