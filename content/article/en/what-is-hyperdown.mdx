...
description: "A full tour of HyperDown: the contentless FTS5 architecture, the generated .hyper-down tree, the hyperdown CLI, the MCP server in Claude Code and opencode, the .agents pack, the test story, and the four create-muttum-app scaffolds."
date: "2026-06-12"
tags:
  [
    "HyperDown",
...
**HyperDown** (`@muttum/hyper-down`) turns a folder of Markdown/MDX files into a typed,
...
It is one of the two independent engines in the **Muttum** toolkit. HyperDown owns
...
- **Server (route loaders)** — `ContentRepository<T>`, exported only from
  `@muttum/hyper-down/server`: `search()` (FTS5 `MATCH` + filters + sort + pagination),
...
(`noExternal: ["@muttum/hyper-down"]`). After a build, `dist/metadata/` holds the final
...
`node_modules/@muttum/hyper-down/.agents/README.md` and the agent inherits the whole
operating manual.
...
`bun create muttum-app` scaffolds a working app already wired to HyperDown + HyperJson,
...
`@muttum/hyper-down/next` adapter (`withHyperDown` + `runHyperDownNextCodegen`) rewrites
