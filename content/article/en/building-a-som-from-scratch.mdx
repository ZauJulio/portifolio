...
```mermaid
flowchart TB
  SOM(["Trained SOM"]) --> P1["Panel 1 · Input data + BMU projections<br/>error vectors, shrinking over training"]
  SOM --> P2["Panel 2 · Weight grid + U-Matrix<br/>geometry and cluster structure together"]
  SOM --> P3["Panel 3 · Activation heatmap<br/>BMU frequency → dead nodes & coverage"]
  SOM --> P4["Panel 4 · 3D view / U-Matrix surface<br/>mesh draped on cloud, or topology as relief"]
...
```mermaid
flowchart TD
  Op["Hot op per epoch<br/>distance matrix · batch weight update"] --> C1{"use_numba AND square<br/>topology AND Euclidean?"}
  C1 -->|"yes"| JIT["@njit(parallel=True)<br/>fused parallel kernel · ~2–5×"]
  C1 -->|"no · e.g. hex or cosine"| NP["NumPy broadcast + linalg.norm<br/>transparent fallback"]
