...
I first ran into Self-Organizing Maps during a scientific initiation project in college, and they stuck with me enough to become the basis of my undergraduate thesis. SOMs do something no other algorithm does quite as cleanly: they learn a **topology-preserving map** from high-dimensional space to a 2D grid — no backpropagation, no labels, no loss function to tune. Neighbors on the grid end up representing similar inputs, and that property alone makes them uniquely useful for visualization and exploration.

<br />

If you want to follow along with the code, the repo is at [**ZauJulio/ZSOM**](https://github.com/ZauJulio/ZSOM) — everything discussed here maps directly to what's in the codebase.

ZSOM is my from-scratch implementation of Kohonen's algorithm. Pure NumPy at the core, optional Numba JIT on the hot path, and a visualization layer that produces publication-quality 4-panel figures and training animations. This post covers the algorithm step by step, the math behind each design choice, the 3D mesh datasets, and the pieces I think push beyond what typical SOM libraries offer.
...
This last property — topology preservation — is what separates SOMs from every other clustering algorithm. k-means produces k disconnected centroids with no spatial relationship between them. A SOM produces a _map_ where proximity on the grid reflects proximity in data space.

```mermaid
flowchart LR
  subgraph IN["Input space · ℝᵈ (high-dim)"]
    direction TB
    d1(("x"))
    d2(("x"))
    d3(("x"))
  ...
  subgraph GRID["SOM grid · w×h prototypes (2D)"]
    direction TB
    g11["W"] --- g12["W"] --- g13["W"]
    g21["W"] --- g22["W"] --- g23["W"]
    g11 --- g21
  ...
  IN ==>|"learn a topology-preserving map"| GRID
  GRID -.-> note["Neighbours on the grid ⇒ similar inputs<br/>proximity is preserved"]
...
Each input point in `ℝᵈ` is mapped to its closest prototype on the flat grid, and because neighbouring nodes are pulled together during training, the 2D layout ends up mirroring the structure of the data. First, let's walk through the algorithm step by step, then we'll dive into the math behind each design choice.

## The Algorithm, Step by Step

The whole training procedure is a tight loop: pick a winner, pull its neighborhood toward
the sample, then cool down. Everything else in this post is a variation on one of these five
steps.

```mermaid
flowchart TD
  Start(["Training data X · n points in ℝᵈ"]) --> Init["1 · Initialize weights W<br/>random or PCA"]
  Init --> BMU

  subgraph epoch["One epoch · for every sample x"]
    direction TB
    BMU["2 · BMU selection — winner-takes-all<br/>bmu = argmin dist(W, x)"]
    Nbr["3 · Neighbourhood<br/>h(i,j,t) = <br/>exp(−d_grid² / 2σ²)"]
    Upd["4 · Weight update<br/>W += η·h·(x − W)"]
    BMU -->|"winner"| Nbr -->|"pull neighbours"| Upd
  end

  Upd --> More{"More samples<br/>this epoch?"}
  More -->|"yes · next x"| BMU
  More -->|"no"| Decay["5 · Decay schedule<br/>η(t) ↓   σ(t) ↓"]
  Decay --> Done{"t = T?"}
  Done -->|"no · next epoch"| BMU
  Done -->|"yes"| Map(["Topology-preserving map"])
...
The neighborhood function determines _how much_ each node moves toward `x`, based on its grid distance from the BMU:
...
<p align="center">
  <img
    ...
    alt="Neighborhood kernel profiles"
  />
</p>
...
<p align="center">
  <img
    ...
    alt="BMU and neighbor update vectors"
  />
</p>
...
<p align="center">
  <img
    ...
    alt="Linear vs exponential decay"
  />
</p>
...
The distance metric is the most fundamental design choice in a SOM.

It defines what "closest node" means in BMU selection. It directly controls the **shape of the Voronoi cells** around each node — the regions of input space that each node "owns." Choosing the wrong metric for your data is equivalent to measuring distance with the wrong ruler.
...
**When to use:** Worst-case analysis. Manufacturing tolerances — a part is out of spec if _any_ measurement exceeds the limit. Board game distances (a king moves one square in any direction — that's Chebyshev distance).

<p align="center">
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/metrics/chebyshev_metric.png"
    alt="Chebyshev metric geometry"
  />
</p>
...
**Effect on the SOM:** The SOM learns a map of _directions_, not positions. Nodes that point toward similar directions in the feature space end up adjacent on the grid. The magnitude of a vector is completely ignored.
...
<p align="center">
  ...
  <img
    ...
    width="28.5%"
    alt="Minkowski metric geometry (p=3)"
  />
</p>
...
Random initialization scatters weights uniformly within the observed range of each feature dimension. If your data lives in a thin ellipsoid in a small region of that hypercube, the SOM spends the first 20–40 epochs just _finding_ the data. PCA init places the weights directly on the data manifold before epoch 1.

### The Math

```mermaid
flowchart LR
  X(["Data X"]) --> A["Center X<br/>subtract the mean"]
  A --> B["SVD<br/>of the covariance"]
  B --> C["Take PC1, PC2<br/>top-2 eigenvectors"]
  C --> D["Build w×h lattice<br/>spanning ±2σ along PC1/PC2"]
  D --> E(["Map back to ℝᵈ<br/>initial weights W"])

  E -.->|"already on the<br/>data manifold"| Fast["≈ epoch 0 looks like<br/>random's epoch 20–40"]
...
<p align="center">
  <img
    ...
    alt="Random vs PCA initialization"
  />
</p>
...
The learning rate and neighborhood radius both decay linearly by default — the classic SOM schedule, and it works well in practice.
...
Read as a per-epoch branch on how the error moved:

```mermaid
flowchart TD
  E["Compute ΔQE = MQE(t) − MQE(t−1)"] --> Q{"compare ΔQE to ε"}
  Q -->|"ΔQE &lt; −ε<br/>steep drop"| Slow["decay × 0.95<br/>ride the momentum longer"]
  Q -->|"abs(ΔQE) &lt; ε<br/>plateau"| Push["decay × 1.05<br/>push to escape the flat"]
  Q -->|"otherwise<br/>normal progress"| Keep["keep the schedule"]
  Slow --> Next(["next epoch"])
  Push --> Next
  Keep --> Next
...
For each node `(i,j)`, compute the average Euclidean distance to its immediate neighbors in _data space_ (not grid space):
...
ZSOM's visualization module produces a 4-panel figure that shows training progress from multiple angles at once:
...
The four panels read the same trained map through four complementary lenses:

<p align="center">
  <img
    src="https://raw.githubusercontent.com/ZauJulio/ZSOM/v2.0.1/__docs__/article/four_panel.png"
    alt="The 4-panel figure: input data with BMU projections, the weight grid colored by U-Matrix, the activation heatmap, and the U-Matrix surface"
  />
</p>
...
I built ZSOM because the existing options were either too simple (50-line demos with one metric) or too opaque (large frameworks where the algorithm disappears behind layers of abstraction). ZSOM tries to sit between those:
...
> SOMs are 35 years old and still produce the most legible unsupervised learning visualizations I know of. The U-Matrix alone — a 2D image where cluster boundaries show up as ridges — communicates more to a non-ML audience in five seconds than any t-SNE plot I've ever shown them.

> This was the first neural network algorithm I worked with in college, during a scientific initiation project that eventually became my undergraduate thesis. Revisiting it years later and building a proper library around it felt like closing a loop. If you try ZSOM and do something interesting with it, I'd love to hear about it — reach out on Twitter or GitHub.
