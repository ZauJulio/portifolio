...
`/pt`). Content engines consumed from npm: `@indago/hyper-down` (Markdown/MDX → SQLite
FTS5, SSR-only) and `@indago/hyper-json` (JSON Schema → typed content) — source at
[ZauJulio/indago](https://github.com/ZauJulio/indago); each installed package ships a
